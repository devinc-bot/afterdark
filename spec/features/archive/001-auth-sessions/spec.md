...
Lumina issues one-day access tokens without persisted sessions, refresh support, or server-side revocation. This change introduces account-scoped sessions for web, dashboard, and admin so authenticated users can renew short-lived access safely, sign out server-side, and retain useful device and approximate location metadata without storing raw refresh credentials.
...
- RF-2: WHEN a refresh credential is issued, THE SYSTEM SHALL send it in an app-specific `HttpOnly` cookie and SHALL persist only its cryptographic hash.
...
- RF-15: THE SYSTEM SHALL obtain geographic metadata through the existing IpQuery provider adapter without coupling authentication use cases to it directly.
...
- RF-22: THE SYSTEM SHALL use host-only refresh cookies named `app.web.auth.refresh`, `app.dashboard.auth.refresh`, and `app.admin.auth.refresh`, with path `/api/auth`, `HttpOnly`, `SameSite=Lax`, a 30-day maximum age, and `Secure` outside local development.
...
- Refresh credentials contain at least 256 bits of cryptographically secure entropy and are never logged or persisted in plaintext.
- Refresh rotation and replay handling are atomic under concurrent requests.
- Supported deployments host `API_PUBLIC_URL`, `WEB_URL`, `DASHBOARD_URL`, and `ADMIN_URL` under the same schemeful site; different ports and subdomains are allowed.
