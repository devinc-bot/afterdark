...
```text
1. @repo/validators (sacar 'de' del enum)
...
| `apps/dashboard/app/modules/owner/hooks/settings-form-context.tsx` | Usa `useLanguage()` de `@repo/i18n/client`. Pasa `currentLanguage` a `createSettingsFormValues` en mount/owner-change. En `save()`: si `preferences.language` elegido difiere del real, llama `applyLanguage()` recién tras el `updateCurrentOwner` exitoso; arma `nextValues` con el idioma recién aplicado. |

## Diseño técnico

- El idioma sigue siendo preferencia 100% cliente (cookie `repo_lang` + `localStorage` de i18next) — no se persiste en `owners` vía API.
...
| ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| 1. `pnpm --filter @repo/validators --filter dashboard type-check`                                                   | Sin errores.                                                                |
