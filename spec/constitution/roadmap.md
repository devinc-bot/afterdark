...
| 004 | `owner-settings`        | Perfil y configuración del dueño     | `done`        | `api`, `dashboard`        | `GET/PATCH /owners/*` consolidado en nuevo módulo `settings` (`SettingsService`, dispatch por rol del JWT). `modules/owner/` → `modules/settings/owner/`; nuevo `modules/settings/staff/` (placeholder "Hello World"). `role` NO viaja en `SessionResponse` — se obtiene de `GET /settings`. Verificado en browser + API real. Contenido real de settings para staff queda para una entrega futura. Ver `spec/features/004-owner-settings/`. |
