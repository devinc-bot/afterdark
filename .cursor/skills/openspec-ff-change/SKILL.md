...
1. **Clarify the request before creating artifacts**
   ...
   Prefer a short set of concrete options (2–4) so the user can decide quickly. Ask only what blocks drafting coherent artifacts.

   **CRITICAL**: Call the **`AskQuestion` tool** so Cursor shows the native multiple-choice UI. Do **not** paste numbered A/B/C lists into the chat message.

   If `AskQuestion` is missing or fails (common with Composer in Agent mode): say so, suggest Plan mode (Shift+Tab) or a non-Composer model, and only then fall back to text.
