...
6. **Implement ONE task, then pause**
   ...
   Default: implement **exactly one** pending task per invocation / turn. Do **not** ship an entire large feature in one apply pass.
   ...
   **Continue automatically only if** the user explicitly says so in this turn, e.g. "seguí", "siguiente", "todas", "hacelo todo", "de una", "continue", "all tasks", or approves a specific batch ("hacé las 2 y 3").
