...
vi.mock('~/modules/common/constants/query-keys', () => ({
  QUERY_KEYS: { accountSessions: () => ['account-sessions'] },
}))

import { AccountSessionsView } from '../app/modules/settings/components/account-sessions-view'

const remoteSession: AccountSessionResponse = {
  documentId: 'admin-remote-session',
...
function renderView() {
  ...
  render(
    createElement(QueryClientProvider, { client: queryClient }, createElement(AccountSessionsView))
  )
...
test('fetches Spanish sessions, revokes a remote session, then invalidates and refetches the list', async () => {
  ...
  await waitFor(() => expect(screen.getByText('Firefox en Linux')).toBeTruthy())
  ...
  expect(serviceMocks.revokeAccountSession.mock.calls[0][0]).toBe('admin-remote-session')
  await waitFor(() => expect(serviceMocks.getAccountSessions).toHaveBeenCalledTimes(2))
  await waitFor(() => expect(screen.queryByText('Firefox en Linux')).toBeNull())
...
test('retries the failed query and retains the confirmation dialog when revocation fails', async () => {
  ...
  expect(screen.getByText('Firefox en Linux')).toBeTruthy()
  queryClient.clear()
})
