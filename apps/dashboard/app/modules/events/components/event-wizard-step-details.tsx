...
export type EventWizardDetailsValues = EventDetailsFormValues & {
  existingImages: EventImageResponse[]
  eventImg: File[]
}

export type EventWizardDetailsFormHandle = {
...
type EventWizardStepDetailsProps = {
  ref?: React.Ref<EventWizardDetailsFormHandle>
  defaultValues: EventWizardDetailsValues
  onDirtyChange?: (isDirty: boolean) => void
}

function snapshotDetailsValues(values: EventWizardDetailsValues): string {
...
export function EventWizardStepDetails({
  ref,
  defaultValues,
  onDirtyChange,
}: EventWizardStepDetailsProps) {
  ...
  const form = useForm({
    defaultValues,
    onSubmit: () => {
      // Submission is orchestrated by the wizard; this form only validates.
    },
  })
  ...
  return (
    <section className="flex flex-col gap-6" aria-labelledby="event-wizard-step-details">
      <div className="flex flex-col gap-1 border-b border-hairline pb-3">
        <h2 id="event-wizard-step-details" className="font-heading text-sm font-semibold text-ink">
          {t('wizard.stepDetailsTitle')}
        </h2>
        <p className="text-sm text-ink-muted">{t('wizard.stepDetailsDescription')}</p>
      </div>
      ...
      >
        ...
        <form.Field name="name" validators={{ onSubmit: eventFieldSchemas.name }}>
          {(field) => {
            ...
            return (
              <Field label={requiredFieldLabel(t('form.name'))} htmlFor={field.name} error={error}>
                <Input
                  id={field.name}
                  name={field.name}
...
function DetailsDirtyReporter({
  ...
  values: EventWizardDetailsValues
