...
const DASHBOARD_REACCEPTANCE_FIELDS = [
  {
...
export function LegalAcceptancePage() {
  ...
  const fields = DASHBOARD_REACCEPTANCE_FIELDS.filter((field) => staleTypes.includes(field.type))
  ...
  return (
    <PageLayout
      title={t('legalAcceptance.title')}
      description={t('legalAcceptance.description')}
      narrow
    >
      <form
        noValidate
        ...
        onSubmit={(event) => {
          ...
          if (!allChecked) {
            ...
            const firstUnchecked = fields.find((field) => acceptedByType[field.type] !== true)
            if (firstUnchecked) {
              document.getElementById(firstUnchecked.id)?.focus()
            }
            return
          }
