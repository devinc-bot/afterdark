...
const remoteSession: AccountSessionResponse = {
  documentId: 'dashboard-remote-session',
...
test('fetches Spanish sessions, revokes a remote session, then invalidates and refetches the list', async () => {
  ...
  await waitFor(() => expect(screen.getByText('Safari en macOS')).toBeTruthy())
  ...
  expect(serviceMocks.revokeAccountSession.mock.calls[0][0]).toBe('dashboard-remote-session')
  await waitFor(() => expect(serviceMocks.getAccountSessions).toHaveBeenCalledTimes(2))
  await waitFor(() => expect(screen.queryByText('Safari en macOS')).toBeNull())
...
test('retries the failed query and retains the confirmation dialog when revocation fails', async () => {
  ...
  expect(screen.getByText('Safari en macOS')).toBeTruthy()
  queryClient.clear()
})
