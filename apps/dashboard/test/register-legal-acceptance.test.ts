  ...
  useTranslation: () => ({
    ...
    t: (key: string) =>
      ({
        ...
        'register.emailPlaceholder': 'local@eventflow.io',
...
const PUBLISHED_TERMS_TITLE = 'Términos dashboard publicados'
const PUBLISHED_TERMS_TEXT = 'Contenido publicado de términos dashboard'
const PUBLISHED_PRIVACY_TITLE = 'Privacidad dashboard publicada'
const PUBLISHED_PRIVACY_TEXT = 'Contenido publicado de privacidad dashboard'
...
function mockPublishedDocuments(
  ...
  serviceMocks.getPublishedLegalDocumentByType.mockImplementation(async (type: string) => {
    if (type === LEGAL_DOCUMENT_TYPE.TERMS_DASHBOARD) {
      if (!terms) throw new Error('No se encontró el documento publicado')
      return terms
    }
    if (type === LEGAL_DOCUMENT_TYPE.PRIVACY_DASHBOARD) {
...
test('opening the privacy dialog shows the published title and converted content', async () => {
  const queryClient = renderForm()

  await waitFor(() =>
    expect(screen.getByRole('button', { name: 'Leer política de privacidad' })).toBeTruthy()
  )
  fireEvent.click(screen.getByRole('button', { name: 'Leer política de privacidad' }))

  const dialog = await screen.findByRole('dialog')
  expect(within(dialog).getByRole('heading', { name: PUBLISHED_PRIVACY_TITLE })).toBeTruthy()
  expect(within(dialog).getByText(PUBLISHED_PRIVACY_TEXT)).toBeTruthy()
  queryClient.clear()
})
...
test('Google continue assigns a start URL with legalAccepted=true after both boxes are checked', async () => {
  ...
  const startUrl = String(locationAssign.mock.calls[0]?.[0])
  expect(startUrl).toContain('legalAccepted=true')
  expect(startUrl).toContain('role=owner')
  expect(startUrl).toContain('app=dashboard')
  queryClient.clear()
})
...
test('register wiring uses published-by-type routes, query keys, and legalAccepted intent', async () => {
  ...
  expect(googleButtonSource).toContain('USER_ROLE.OWNER')
  expect(googleButtonSource).toContain('AUTH_OAUTH_APP.DASHBOARD')
  ...
  expect(legalService).toContain('DASHBOARD_ROUTES.legalAcceptance')
