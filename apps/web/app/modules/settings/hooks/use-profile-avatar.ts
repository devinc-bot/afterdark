...
import {
  useRemoveMyAvatar,
  useUploadMyAvatar,
} from '~/modules/settings/queries/use-avatar-mutations'
...
export function useProfileAvatar(): {
  ...
  async function handleAvatarSelection(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!isAllowedImageMimeType(file.type)) {
      toast.error(t('web.profile.invalidType'))
      return
    }

    if (file.size > AVATAR_UPLOAD_MAX_BYTES) {
      toast.error(t('web.profile.fileTooLarge'))
      return
    }
    ...
    try {
      ...
      toast.error(t('web.profile.uploadError'))
    }
  }
  ...
  async function handleAvatarConfirm(avatar: Blob) {
    try {
      ...
      setCropImageSrc(null)
      toast.success(t('web.profile.uploadSuccess'))
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t('web.profile.uploadError'))
    }
  }
  ...
  async function handleAvatarRemove() {
    try {
      ...
      toast.success(t('web.profile.removeSuccess'))
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t('web.profile.removeError'))
    }
  }
