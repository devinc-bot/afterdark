...
import * as profileServiceModule from '../app/modules/settings/services/profile.service'
...
const profileAvatarService = profileServiceModule as typeof profileServiceModule &
...
function requireServiceMethod<Key extends keyof ProfileAvatarService>(
  ...
  expect(method, `profile.service must export ${key}`).toBeTypeOf('function')
...
describe('web profile avatar service', () => {
  test('uploads the cropped avatar through the dedicated multipart PUT endpoint', async () => {
    ...
    const response = { avatar: 'https://cdn.example.test/users/avatars/user.webp' }
