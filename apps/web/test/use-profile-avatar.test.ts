...
vi.mock('~/modules/settings/queries/use-avatar-mutations', () => ({
...
import { useProfileAvatar } from '../app/modules/settings/hooks/use-profile-avatar'
...
describe('useProfileAvatar', () => {
  ...
  test('rejects invalid avatar types and files exceeding the size limit', async () => {
    ...
    expect(toastMocks.error).toHaveBeenNthCalledWith(1, 'web.profile.invalidType')
    expect(toastMocks.error).toHaveBeenNthCalledWith(2, 'web.profile.fileTooLarge')
    expect(createObjectURL).not.toHaveBeenCalled()
    expect(mutationMocks.upload.reset).not.toHaveBeenCalled()
  })
  ...
  test('revokes an undecodable image and reports an upload error', async () => {
    ...
    expect(revokeObjectURL).toHaveBeenCalledWith('blob:avatar')
    expect(toastMocks.error).toHaveBeenCalledWith('web.profile.uploadError')
  })
  ...
  test('uploads the confirmed crop, closes it, and reports success', async () => {
    ...
    expect(result.current.cropImageSrc).toBeNull()
    expect(toastMocks.success).toHaveBeenCalledWith('web.profile.uploadSuccess')
  })
  ...
  test('reports upload failures using the error message or translated fallback', async () => {
    ...
    expect(toastMocks.error).toHaveBeenNthCalledWith(2, 'web.profile.uploadError')
  })
  ...
  test('removes an avatar, closes its dialog, and reports success', async () => {
    ...
    expect(toastMocks.success).toHaveBeenCalledWith('web.profile.removeSuccess')
  })
  ...
  test('keeps the remove dialog open and reports mutation failures', async () => {
    ...
    expect(toastMocks.error).toHaveBeenNthCalledWith(2, 'web.profile.removeError')
  })
