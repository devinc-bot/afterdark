...
Owner organization profile on dashboard settings: declare organization mode, persist organization name on the owner row, and manage CUIT (`taxId`) in the organization section rather than personal profile.
...
The dashboard owner settings page SHALL include an Organización section with the question “¿Eres una organización?” and mutually exclusive Sí / No controls (No selected by default when the owner has no organization data). When Sí is selected, the section MUST show inputs for organization name and CUIT (`taxId`). When No is selected, those inputs MUST be hidden. The CUIT field MUST NOT appear in the personal profile section. UI chrome (section title, question, labels, hints) MUST be Spanish via `@repo/i18n` with English parity. Organization mode MUST NOT be persisted as its own database column.

#### Scenario: Organization mode on when organization data exists

- **GIVEN** the current owner has a non-empty `organizationName` or a non-empty `taxId`
- **WHEN** the owner opens dashboard settings
- **THEN** Sí is selected and the organization name and CUIT inputs are visible (prefilled from the API)

#### Scenario: Organization mode off when no organization data

- **GIVEN** the current owner has null/empty `organizationName` and null/empty `taxId`
- **WHEN** the owner opens dashboard settings
- **THEN** No is selected and the organization name and CUIT inputs are not shown

#### Scenario: Selecting Sí reveals organization fields

- **WHEN** the owner selects Sí
- **THEN** the organization name and CUIT inputs become visible

#### Scenario: Selecting No hides and clears form fields before save
...
When the owner submits settings as an organization (Sí selected / organization fields path), `organizationName` MUST be required per `@repo/validators`. `taxId` MUST remain optional subject to the existing validator rules when provided.

#### Scenario: Submit as organization without name fails

- **WHEN** the owner has Sí selected and submits with an empty organization name
...
- **WHEN** the owner has Sí selected, provides a valid organization name, leaves CUIT empty, and submits
